...
- (void)requestUserDataWithVerifier:(NSString *)verifier {
    ...
    NSDictionary *parameters = @{@"client_id" : kClientID,
                                 ...
                                 @"code" : verifier};
    [manager POST:kBaseURLWithOauthAndToken parameters:parameters
...
- (void)saveUserDataAndLoadFirstPackOfPosts:(NSDictionary *)userData {
   __block SPPUserDataModel *newUser = nil;
    [MagicalRecord saveWithBlockAndWait:^(NSManagedObjectContext *localContext){
        NSArray *existedUsers = [SPPUserDataModel MR_findAllWithPredicate:[NSPredicate predicateWithFormat:@"accessToken == %@", userData[@"access_token"]]];
